...
一开始会从网络层获取请求文档的内容，请求过来的数据是 Bytes，然后浏览器将其编译成HTML的代码。
...
就像html解析出来的parse tree并不是严格意义上的dom tree 一样。css 解析出来得到的 parse tree 并不是严格等于CSSOM tree，而是经过改造之后的，主要是为了浏览器的使用。
...
每一个`CSSStyleDeclaration`节点里面会有所有可能出现的css 属性。对于没有制定样式的属性，其值为`""`。
...
在2011年的时候，浏览器厂商推出了“推测性”解析。

“推测性”解析就是，当让渲染引擎干等着js代码下载和运行的时候，会起一个第三个进程，继续解析剩下的html。当js代码加载好了，就会马上发起对剩下html所引用的资源——图片，样式表和js代码的请求。

这样就节省了之后加载和解析时间。

被称为“推测性”解析是因为，前面的js代码会修改DOM节点，有可能会让后面的DOM节点消逝，那么我们的工作就白费了。浏览器“推测”这样的发生的概率比较小。



### Render Tree

当浏览器忙着构建dom tree，和 CSSOM tree的时候,  浏览器在同时一边将两者结合生成render tree。 这两个是同时进行的。

`render tree`是可以被展示的那些元素，按照被展示的顺序排列起来。

`render tree` 上面的元素被称为 `renderer`, 或者 被称为 `render objects`, 不同的浏览器叫法不同。这篇文章中，我们统一叫法是 **Render-Tree **的节点。

需要注意的是，`render objects` 都长方形。



webkit 有一个类，`RenderObject.h`， 这个是所有render对象的父类。
...
class RenderObject {
  ...
  Node* node;  // The DOM node this RenderObject is attached to.
  RenderStyle* style;  // This RenderObject's computed styles.
  RenderLayer* containingLayer;  // The z-index layer that contains this RenderObject.
}
```
